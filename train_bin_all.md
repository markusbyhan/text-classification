...
train_prep_comp_4 <- germ_comp_clean %>%
  select(A001:A012)

# Function for decision-making based on all annotators
all_non_zero <- function(train_prep_comp_4) {
  # Apply the function to each row of the dataframe
  apply(train_prep_comp_4, 1, function(row) {
    # Remove NA values
...
results_all <- all_non_zero(train_prep_comp_4)

# Add the results as a new column `bin_all` to train_prep_comp_4
train_prep_comp_4$bin_all <- results_all
train_prep_comp_4$bin_all <- as.factor(train_prep_comp_4$bin_all)
class(train_prep_comp_4$bin_all)
```
