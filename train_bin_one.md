...
train_prep_comp_3 <- germ_comp_clean %>%
  select(A001:A012)

# Function to check for at least 1 non-zero value
one_non_zero <- function(train_prep_comp_3) {
  # Apply the function to each row of the dataframe
  apply(train_prep_comp_3, 1, function(row) {
    # Remove NA values
...
results_one <- one_non_zero(train_prep_comp_3)

# Add the results as a new column `bin_one` to train_prep_comp_3
train_prep_comp_3$bin_one <- results_one
train_prep_comp_3$bin_one <- as.factor(train_prep_comp_3$bin_one)
