...
train_prep_6 <- train_prep %>%
  select(A001:A012)

# Function to check disagreement between '0' and other labels
disagreement <- function(train_prep_6) {
  apply(train_prep_6, 1, function(row) {
    # Remove NA values
...
results_disagree <- disagreement(train_prep_6)

# Add the results as a new column `disagree_bin` to train_prep_6
train_prep_6$disagree_bin <- results_disagree
train_prep_6$disagree_bin <- as.factor(train_prep_6$disagree_bin)
class(train_prep_6$disagree_bin)
```
