...
tf <- as.matrix(dtm_train) # Converts the DTM into a matrix format
row_sums <- rowSums(tf)    # Total term counts for each document

# Calculate the document frequency (DF)
df <- colSums(tf > 0)      # Number of documents each term appears in

# Calculate the inverse document frequency (IDF)
idf <- log2(nDocs(dtm_train) / df) # Applying the formula for IDF

# Calculate TF-IDF
tf_idf <- sweep(tf, 2, idf, FUN = "*") # Multiply TF by IDF for each term

# Convert the TF-IDF matrix into a DataFrame for easier interpretation
